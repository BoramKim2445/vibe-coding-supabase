...
const mockUserData: UserProfile = {
  ...
  joinDate: "2024.03",
  transactionKey: "test-transaction-key-12345"
};
...
function GlossaryMagazinesMypage() {
  ...
  const handleCancelSubscription = async () => {
    ...
    if (confirm("구독을 취소하시겠습니까?")) {
      try {
        await cancelPayment({ transactionKey: user.transactionKey });
        // 성공 시 Hook에서 알림과 페이지 이동을 처리합니다
      } catch (error) {
        // 에러는 Hook에서 처리되므로 여기서는 별도 처리 불필요
